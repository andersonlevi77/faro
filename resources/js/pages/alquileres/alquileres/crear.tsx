...
export default function AlquileresCrear({
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        deposito_monto: '',
        notas: '',
        lineas: [{ producto_id: '' as number | '', cantidad: '1' }] as LineaForm[],
    });
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        <div className="space-y-2 sm:col-span-2">
                            ...
                            <select
                                ...
                                onChange={(e) =>
                                    setData('cliente_id', e.target.value === '' ? '' : Number(e.target.value))
                                }
                            >
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="deposito_monto">Depósito / fianza</Label>
                            <Input
                                id="deposito_monto"
                                type="number"
                                step="0.01"
                                        ...
                                        >
                                            ...
                                            {productosAlquiler.map((p) => (
                                                <option key={p.id} value={p.id}>
                                                    {p.nombre} (stock {p.stock_alquiler} · {fmtQ(p.precio_alquiler_diario ?? '0')}/día)
                                                </option>
                                            ))}
                                        </select>
                                    </div>
