            ...
            <div className="flex h-full flex-1 flex-col gap-5 p-4 md:p-5">
                ...
                <form onSubmit={handleSubmit} className="w-full space-y-6 rounded-xl bg-card p-6 shadow-[0_1px_3px_0_rgba(0,0,0,0.04)]">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input id="codigo_barras" value={data.codigo_barras} onChange={(e) => setData('codigo_barras', e.target.value)} />
                        <InputError message={errors.codigo_barras} />
                    </div>
                    <div className="grid gap-4 sm:grid-cols-3">
                        <div className="space-y-2">
                            <Label>Categoría</Label>
                            <Select value={data.categoria_id ? String(data.categoria_id) : ''} onValueChange={(v) => setData('categoria_id', v ? Number(v) : '')}>
                        ...
                        <div className="space-y-2">
                            <Label>Marca</Label>
                            <Select value={data.marca_id ? String(data.marca_id) : ''} onValueChange={(v) => setData('marca_id', v ? Number(v) : '')}>
                                <SelectTrigger><SelectValue placeholder="Seleccionar" /></SelectTrigger>
                                <SelectContent>
                        ...
                        <div className="space-y-2">
                            ...
                            <Select value={data.presentacion_id ? String(data.presentacion_id) : ''} onValueChange={(v) => setData('presentacion_id', v ? Number(v) : '')}>
                                <SelectTrigger><SelectValue placeholder="Seleccionar" /></SelectTrigger>
                                <SelectContent>
                                    {presentaciones.map((p) => <SelectItem key={p.id} value={String(p.id)}>{p.nombre}</SelectItem>)}
                                </SelectContent>
                            </Select>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        <div className="space-y-2">
                            ...
                            <Input id="precio_compra" type="number" step="0.01" min="0" value={data.precio_compra} onChange={(e) => setData('precio_compra', e.target.value)} required />
                            <InputError message={errors.precio_compra} />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="precio_venta">Precio de venta *</Label>
                            <Input id="precio_venta" type="number" step="0.01" min="0" value={data.precio_venta} onChange={(e) => setData('precio_venta', e.target.value)} required />
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        <div className="space-y-2">
                            ...
                            <Input id="stock_minimo" type="number" min="0" value={data.stock_minimo} onChange={(e) => setData('stock_minimo', e.target.value)} />
                            <InputError message={errors.stock_minimo} />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="stock_maximo">Stock máximo</Label>
                            <Input id="stock_maximo" type="number" min="0" value={data.stock_maximo} onChange={(e) => setData('stock_maximo', e.target.value)} />
                    ...
                    <div className="space-y-4 rounded-lg border border-border/60 bg-muted/10 p-4">
                        ...
                        <div className="flex items-center gap-2">
                            <input type="checkbox" id="es_alquilable" checked={data.es_alquilable} onChange={(e) => setData('es_alquilable', e.target.checked)} className="size-4 rounded border-input" />
                            ...
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        {data.tracking_mode === 'individual'
                                            ? 'Cada unidad física tiene código único, estado propio e historial.'
                                            : 'Se controla por cantidad total disponible.'}
                                    </p>
                                ...
                                <div className="grid gap-4 sm:grid-cols-2">
                                    <div className="space-y-2">
                                        ...
                                        <Input id="precio_alquiler_diario" type="number" step="0.01" min="0" value={data.precio_alquiler_diario} onChange={(e) => setData('precio_alquiler_diario', e.target.value)} />
                                        <InputError message={errors.precio_alquiler_diario} />
                                    </div>
                                    <div className="space-y-2">
                                        <Label htmlFor="deposito_unitario">Depósito / garantía</Label>
                                        <Input id="deposito_unitario" type="number" step="0.01" min="0" value={data.deposito_unitario} onChange={(e) => setData('deposito_unitario', e.target.value)} placeholder="Monto por unidad" />
                                ...
                                {data.tracking_mode === 'bulk' && (
                                    <div className="space-y-2">
                                        ...
                                        <Input id="stock_alquiler" type="number" step="0.001" min="0" value={data.stock_alquiler} onChange={(e) => setData('stock_alquiler', e.target.value)} />
                                ...
                                {data.tracking_mode === 'individual' && (
                                    <p className="text-xs text-muted-foreground bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-md p-3">
                                        Las unidades físicas se gestionan desde la ficha del producto.
                                    </p>
                                )}
                            </div>
                        )}
